        ...
        def main():
            if parameters["-k"] not in ["6","7","8"]:
                ...
                if parameters["-s"]=="1":
                    ...
                    def fix_stamp(myfile):
                        h={}
                        f=open(myfile)
                        o=open(myfile.replace(".txt",".spf"),"w+")
                        o.write(f.readline())
                        for line in f:
                            line=line.split("\t")
                            genus=line[5]
                            ...
                            if genus not in h:
                                ...
                                o.write(h[genus]+"\t"+"\t".join(line[5:]))
                                 
                        f.close()
                        o.close()

                    fix_stamp(outputname+"__STAMP_tabular.txt")
                    os.system("rm "+outputname+"__STAMP_tabular.txt")
                    ...
                    for i in xrange(7):
                        ...
                        with open(myFile, 'w') as o:
                             [o.write(line) for line in currentTab]
